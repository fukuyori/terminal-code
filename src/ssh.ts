...
export function sshOpen(
  ...
  const bundle = writeBundle(options.palette, upstreamSshVersion(options.version), options.remotePath);
  ...
  const command = commandWith(runtime.command, argv);
...
/** A Windows fork release runs the matching upstream build on the remote
 * machine. Upstream release versions keep their leading `v`, while Windows
 * release versions use a `-win.N` suffix. */
export function upstreamSshVersion(version: string): string {
  const windows = /^v?(\d+\.\d+\.\d+)-win\.\d+$/.exec(version);
  return windows ? `v${windows[1]}` : version;
}
